...
// Close Files, a separator, Settings…) followed by view.help's own Help
// menu - mirroring help's own TestHelpMenu (manual_test.go), which covers
// the Help submenu's own contents.
func TestBuildMainMenu_Structure(t *testing.T) {
	...
	if len(file.Items) != 4 {
		t.Fatalf("File menu items = %d, want 4 (Open Files…, Close Files, separator, Settings…)", len(file.Items))
	}

	if got := file.Items[0]; got.Label != "Open Files…" || got.Action == nil {
		t.Errorf("File menu item 0 = %+v, want %q with an action", got, "Open Files…")
	}
	if got := file.Items[1]; got.Label != "Close Files" || got.Action == nil {
		t.Errorf("File menu item 1 = %+v, want %q with an action", got, "Close Files")
	}
	if !file.Items[2].IsSeparator {
		t.Error("expected a separator between Close Files and Settings…")
	}
	if got := file.Items[3]; got.Label != "Settings…" || got.Action == nil {
		t.Errorf("File menu item 3 = %+v, want %q with an action", got, "Settings…")
	}
