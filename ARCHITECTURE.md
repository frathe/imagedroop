...
| `menu.go`        | `buildMainMenu` — the window's menu bar: a File menu (Open Files…/Save Changes/Export as PNG…/Export as JPEG…/Close Files/Settings…, built here) composed with `help.Menu()`'s Help menu. The one place that decides how the two compose, per the cross-feature-composition rule below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `stubs.go`  | `StubChooser`, `StubSaveChooser`, `StubClipboardCopy`, `StubTrashMove` — swap `filepicker.Choose`/`filepicker.ChooseSave`/`clipboard.CopyImage`/`trash.Move` for the duration of a test |
...
- "Which keys do what?" → `keys.go`'s `handleKeyEvent` (key names) and `handleTypedRune` (typed characters, grid search only)
