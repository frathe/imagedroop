...
| `stubs.go`  | `StubChooser`, `StubSaveChooser`, `StubClipboardCopy`, `StubClipboardCopyFiles`, `StubTrashMove` — swap `filepicker.Choose`/`filepicker.ChooseSave`/`clipboard.CopyImage`/`clipboard.CopyFiles`/`trash.Move` for the duration of a test |
...
- "How does delete work?" → `internal/ui/deletion` (the flow, single and batch) + `internal/trash` (per-OS
  move-to-Trash) + `build.go`'s `wireDeleteShortcut` and `batch.go`'s `requestDelete` (how Shift+Delete reaches it,
