...
vector re-render's `vectorGen`/`vectorStop` (see `vector.go`), and the grid's and viewer's
...
| File        | Responsibility                                                                                                                                              |
|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `uitest.go` | `TempJPEGURI`, `WriteTempFile`, `EncodeJPEG`/`EncodePNG`/`EncodeGIF`/`EncodeAnimatedGIF`, `SVGBytes`/`TempSVGURI` (a synthetic SVG with a given viewBox, so a rasterization of it has visibly non-zero pixels), `CaptureDateJPEG`, `TruncatedPNGHeader`, `FakeURI`, `ApproxEqual` |
